...
class CategoryController extends PublicController {
    ...
    public function index(){
        ...
        if($_REQUEST['cate_id']){
            ...
            $catList = M('category')->where('tid="'.$tid.'"')->field('id,name,bz_1')->select();
             ...
             $catList = M('category')->where('tid=2')->field('id,name,bz_1')->select();
        }
    ...
    public function getProduct(){
        ...
        if($catid){
            ...
            if($catid == 2){
                $pro = M('product')->where('del=0 AND is_down=0 AND pro_type=1')->select();
            }
        }else if($key){
       ...
       if(!empty($_REQUEST['num'])){
        ...
        //json加密输出
        //dump($json);
        $category = M('category')->where('tid!=0')->field('id,name,bz_1')->select();
        if (empty($pro)) {
